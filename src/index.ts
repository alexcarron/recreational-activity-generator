...
function addEnumOptionsToSelection(
	select_elem: HTMLSelectElement,
	Enum: {[property: string]: any},
	default_option: string | number | undefined = undefined
) {
	...
	for (let enumerator of Object.values(Enum)) {
		if (typeof enumerator === "number") {
			...
			const enum_txt: string = ToCase.titleCase(enumerator);
			...
			if (default_option === enumerator) {
				const option_elm: HTMLOptionElement | null = document.querySelector(`option[value="${enumerator}"]`);
				if (option_elm) {
...
function showActivities(activities_section: Element, ...activities: Activity[]): void {
	activities_section.innerHTML = "";

	for (const item of activities) {
		const activity: Activity = item;
		const activity_wrapper: HTMLDivElement = getActivityWrapper(activity);
		activities_section.append(activity_wrapper);
	}
}

function getActivityWrapper(activity: Activity) {
	const activity_wrapper = document.createElement("div");
	activity_wrapper?.classList.add("activity_wrapper");

	{
		const header = document.createElement("header");
		const name_heading = document.createElement("h2");
		name_heading?.classList.add("name_heading");
	...
	if (activity.description !== "") {
		...
		description_heading?.classList.add("description_heading");

		const description_paragraph = document.createElement("p");
		description_paragraph?.classList.add("description_paragraph");
...

	{
		...
		duration_range_section?.classList.add("duration_range_section");

		const duration_range_heading = document.createElement("h3");
		duration_range_heading?.classList.add("duration_range_heading");

		const duration_range_ul = document.createElement("ul");
		duration_range_ul?.classList.add("duration_range_ul");

		const minimum_duration_li = document.createElement("li");
		minimum_duration_li?.classList.add("minimum_duration_li");

		const maximum_duration_li = document.createElement("li");
		maximum_duration_li?.classList.add("maximum_duration_li");
	...
	if (activity.required_focuses.length > 0) {
		...
		required_focus_section?.classList.add("required_focus_section");

		const required_focus_heading = document.createElement("h3");
		required_focus_heading?.classList.add("required_focus_heading");

		const required_focus_ul = document.createElement("ul");
		required_focus_ul?.classList.add("required_focus_ul");

		required_focus_heading.textContent = "Focuses Requried";

		activity.required_focuses.forEach(
			(focus: Focus) => {
				const focus_li = document.createElement("li");
				focus_li?.classList.add("focus_li");
	...
	if (activity.required_devices.length > 0) {
		...
		required_items_section?.classList.add("required_items_section");

		const required_items_heading = document.createElement("h3");
		required_items_heading?.classList.add("required_items_heading");

		const required_items_ul = document.createElement("ul");
		required_items_ul?.classList.add("required_items_ul");

		required_items_heading.textContent = "Required Items";

		activity.required_devices.forEach(
			(item: Item) => {
...

	{
		...
		num_participants_section?.classList.add("num_participants_section");

		const num_participants_heading = document.createElement("h3");
		num_participants_heading?.classList.add("num_participants_heading");

		const num_participants_ul = document.createElement("ul");
		num_participants_ul?.classList.add("num_participants_ul");

		const min_participants_li = document.createElement("li");
		min_participants_li?.classList.add("min_participants_li");

		const max_participants = document.createElement("li");
		max_participants?.classList.add("max_participants_li");
...

	{
		...
		internet_section?.classList.add("internet_section");

		const internet_heading = document.createElement("h3");
		internet_heading?.classList.add("internet_heading");

		const internet_paragraph = document.createElement("p");
		internet_paragraph?.classList.add("internet_paragraph");
...

	{
		...
		productive_section?.classList.add("productive_section");

		const productive_heading = document.createElement("h3");
		productive_heading?.classList.add("productive_heading");

		const productive_paragraph = document.createElement("p");
		productive_paragraph?.classList.add("productive_paragraph");
...

	{
		...
		enjoyment_level_section?.classList.add("enjoyment_level_section");

		const enjoyment_level_heading = document.createElement("h3");
		enjoyment_level_heading?.classList.add("enjoyment_level_heading");

		const enjoyment_level_paragraph = document.createElement("p");
		enjoyment_level_paragraph?.classList.add("enjoyment_level_paragraph");
...

	{
		...
		location_section?.classList.add("location_section");

		const location_heading = document.createElement("h3");
		location_heading?.classList.add("location_heading");

		const location_paragraph = document.createElement("p");
		location_paragraph?.classList.add("location_paragraph");
...
function main() {
	...
	if (activities_section !== null) {
		showActivities(activities_section, ...(globalThis as any).activities);
	}

	const focus_select = <HTMLSelectElement> document.querySelector("select#focus_select");
	const items_select = <HTMLSelectElement> document.querySelector("select#items_select");
	const required_place_select = <HTMLSelectElement> document.querySelector("select#required_place_select");
	const min_duration_unit_select = <HTMLSelectElement> document.querySelector("select#min_duration_unit_select");
	const max_duration_unit_select = <HTMLSelectElement> document.querySelector("select#max_duration_unit_select");
	const enjoyment_level_select = <HTMLSelectElement> document.querySelector("select#enjoyment_level_select");
	const is_productive_select = <HTMLSelectElement> document.querySelector("select#is_productive_select");
	const num_buttons: NodeListOf<HTMLButtonElement> = document.querySelectorAll("button.num_button");
	...
	num_buttons.forEach(
		(num_button) => {
			...
			num_button.onclick = () => {
				const num_adding: number = parseInt(num_button.textContent ? num_button.textContent : "0");
				const for_element_name = num_button.getAttribute("for");
				const for_element: HTMLInputElement = <HTMLInputElement> document.querySelector(`[name="${for_element_name}"]`);
				...
				if (for_element) {
					...
					if (for_element.textContent) {
						num = parseInt(for_element?.textContent);
					}
	...
	if (is_productive_select !== null) addEnumOptionsToSelection(is_productive_select, YesNoCheckbox, YesNoCheckbox.Either);
	...
	function addActivity() {
		...
		const name_input = <HTMLInputElement> document.querySelector("input#activity_name_input");
		const description_input = <HTMLInputElement> document.querySelector("input#activity_description_input");
		const min_duration_amount_input = <HTMLInputElement> document.querySelector("input#min_duration_amount_input");
		const min_duration_unit_select = <HTMLSelectElement> document.querySelector("select#min_duration_unit_select");
		const max_duration_amount_input = <HTMLInputElement> document.querySelector("input#max_duration_amount_input");
		const max_duration_unit_select = <HTMLSelectElement> document.querySelector("select#max_duration_unit_select");
		const focus_select = <HTMLSelectElement> document.querySelector("select#focus_select");
		const items_select = <HTMLSelectElement> document.querySelector("select#items_select");
		const min_participants_input = <HTMLInputElement> document.querySelector("input#min_participants_input");
		const max_participants_input = <HTMLInputElement> document.querySelector("input#max_participants_input");
		const need_internet_checkbox = <HTMLInputElement> document.querySelector("input#need_internet_checkbox");
		const is_productive_checkbox = <HTMLInputElement> document.querySelector("input#is_productive_checkbox");
		...
		duration_range = new NumRange<Duration>(
			new Duration(
				parseInt(min_duration_amount_input.value),
				getEnumFromValue(UnitOfTime, min_duration_unit_select.selectedOptions[0].value)
			),
			new Duration(
				parseInt(max_duration_amount_input.value),
				getEnumFromValue(UnitOfTime, max_duration_unit_select.selectedOptions[0].value)
			)
		);

		Array.from(focus_select.selectedOptions).map(
			(option_element) => {
		...
		Array.from(items_select.selectedOptions).map(
			(option_element) => {
		...
		num_participants_needed = new NumRange<number>(
			parseInt(min_participants_input.value),
			parseInt(max_participants_input.value)
		);

		needInternet = need_internet_checkbox.checked;

		isProductive = getEnumFromValue(
			YesNoCheckbox,
			Parser.parseStrFromBoolean(
				is_productive_checkbox.checked,
				YesNoCheckbox.Yes,
				YesNoCheckbox.No
			)
		);
		...
		(globalThis as any).activities.push(activity);
		console.log((globalThis as any).activities);
	...
	function getFilteredActivities() {
		...
		let duration_range: NumRange<Duration>,
			required_focuses: Focus[] = [],
			required_devices: Item[] = [],
			num_participants_needed: NumRange<number>,
			needInternet: boolean,
		...
		const is_productive_select = <HTMLSelectElement> document.querySelector("select#is_productive_select");
		// const enjoyment_level_input = <HTMLInputElement> document.querySelector("input#enjoyment_level_input");
		const enjoyment_level_input = <HTMLSelectElement> document.querySelector("select#enjoyment_level_select");
		const required_place_select = <HTMLSelectElement> document.querySelector("select#required_place_select");
		...
		const filtered_activities: Activity[] = activities.filter(
			(activity) => {
				...
				if (
					duration_range.min.isLargerThanDuration(activity.duration_range.max) ||
				...
				if (
					num_participants_needed.min > activity.num_participants_needed.max ||
				...
				if (
					isProductive !== YesNoCheckbox.Either &&
	...
	function searchForActivityButtonEvent() {
		const filtered_activities: Activity[] = getFilteredActivities();
	...
	function generateRandomActivityButtonEvent() {
		...
		if (filtered_activities.length <= 0) {
			...
			const random_activity: Activity = filtered_activities[random_activity_index];
	...
	const add_activity_button = <HTMLButtonElement> document.querySelector("button#add_activity_button");
	if (add_activity_button !== null) add_activity_button.onclick = addActivity;

	const search_for_activity_button = <HTMLButtonElement> document.querySelector("button#search_for_activity_button");
	if (search_for_activity_button !== null) {search_for_activity_button.onclick = searchForActivityButtonEvent};

	const random_activity_button = <HTMLButtonElement> document.querySelector("button#random_activity_button");
	if (random_activity_button !== null) random_activity_button.onclick = generateRandomActivityButtonEvent;
